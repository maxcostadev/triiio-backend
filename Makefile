...
help:
	...
	@echo "  make import-properties    - Import properties from external API"
	@echo ""
	...
	@echo "  make test-migrations             - Test migrations from scratch (development only!)"
	@echo ""
...
## test-migrations: Test all migrations from scratch (⚠️  DEVELOPMENT ONLY - DROPS ALL DATA!)
test-migrations:
	@echo "⚠️  WARNING: This will DROP all imoveis-related tables and reapply migrations!"
	@echo "⚠️  This should ONLY be used in DEVELOPMENT environments!"
	@echo ""
...
## import-properties: Import properties from external API
import-properties:
ifdef CONTAINER_RUNNING
	@echo "$(ENV_MSG)"
	@docker exec -it $(CONTAINER_NAME) go run cmd/importimoveis/main.go
else
	@if command -v go >/dev/null 2>&1; then \
		echo "$(ENV_MSG)"; \
		go run cmd/importimoveis/main.go; \
	else \
		echo "❌ Error: Docker container not running and Go not installed"; \
		echo "Please run: make up"; \
		exit 1; \
