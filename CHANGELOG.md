...
#### 🏠 Property Management (Imoveis)
- ✨ **External ID Mapping System** - Robust integration with external property APIs
  - Added `id_integracao` field to all relationship tables (empreendimentos, preco_vendas, preco_alugueis, organizacoes, pacotes)
  - Format: `EXT_{external_id}` for tracking external API records
  - Unique indexes on `id_integracao` for fast lookups
  - Migration: `20260114211500_add_id_integracao_to_related_tables`

- ✨ **Upsert Logic for Import System** - Smart create-or-update functionality
  - Automatic detection of existing records via `id_integracao`
  - Create new records when not found
  - Update existing records with latest data from external API
  - Only business-relevant fields updated (preserves `created_at`, updates `updated_at`)
  - Applied to: Empreendimento, PrecoVenda, PrecoAluguel, Imovel

- ✨ **Attachment Deduplication** - Prevents duplicate images on re-import
  - URL-based duplicate detection
  - Skips existing attachments automatically
  - Reduces storage and database overhead

- ✨ **Enhanced Foreign Key Handling** - Robust constraint management
  - Dynamic `Omit()` in repository layer for zero-value foreign keys
  - Prevents constraint violations for nullable relationships
  - Applied to: EmpreendimentoID, PlantaID, PrecoVendaID, PrecoAluguelID, OrganizacaoID, PacoteID
  - Repository `Update()` method skips associations to prevent cascade updates

### Fixed

#### 🐛 Import System
- Fixed PostgreSQL DATE type errors with empty strings from external API
- Fixed foreign key constraint violations during property updates
- Fixed duplicate attachment creation on subsequent imports
- Fixed GORM association update issues in repository layer

### Changed

#### 📝 Documentation
- Updated IMPORT_GUIDE.md with id_integracao mapping strategy
- Added upsert behavior documentation (create vs update scenarios)
- Documented error handling patterns and solutions
- Added best practices for import re-runs
